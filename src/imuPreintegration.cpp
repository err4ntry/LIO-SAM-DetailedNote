...
public:
    ...
    */
    Eigen::Affine3f odom2affine(nav_msgs::Odometry odom)
    {
        ...
        return pcl::getTransformation(x, y, z, roll, pitch, yaw);//车体坐标系到大地坐标系的旋转矩阵
    }
    ...
    */
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        tf::Transform tCur;
        tf::poseMsgToTF(laserOdometry.pose.pose, tCur);// 将geometry_msgs::Pose的类型转化为tf::Transform类
        if(lidarFrame != baselinkFrame)
...
public:
    ...
    */
    IMUPreintegration()
    {
        ...
        noiseModelBetweenBias = (gtsam::Vector(6) << imuAccBiasN, imuAccBiasN, imuAccBiasN, imuGyrBiasN, imuGyrBiasN, imuGyrBiasN).finished();//bias噪声
    ...
    */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)//利用输入的lidar pose和IMU数据进行图优化，获得一个最合理的状态量估计值，包括PVQ，各种noise，bias。
    {
        std::lock_guard<std::mutex> lock(mtx);
        // 当前帧激光里程计时间戳
        ...
        bool degenerate = (int)odomMsg->pose.covariance[0] == 1 ? true : false;//这个东西不是方差，放了一个标志位，告诉你这个lidar pose准不准，1是不准，0是准，分别对应了不同的方差。
        ...
        while (!imuQueOpt.empty())//首先把IMU数据导入到用于优化的IMU预积分器里：
        {
        ...
        gtsam::PriorFactor<gtsam::Pose3> pose_factor(X(key), curPose, degenerate ? correctionNoise2 : correctionNoise);//根据位姿准不准，它添加的协方差是不一样的 ?
    ...
    */
    ...
    {
        std::lock_guard<std::mutex> lock(mtx);
        // imu原始测量数据转换到lidar系，加速度、角速度、RPY//imuConverter函数，转换到一个“雷达中间系”中，其实和真正的雷达系差了一个平移
