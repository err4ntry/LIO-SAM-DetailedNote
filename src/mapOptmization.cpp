...
public:
    ...
    */
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        if (detectLoopClosureExternal(&loopKeyCur, &loopKeyPre) == false)// not-used
    ...
    */
    void updateInitialGuess()
    {
        ...
        {
            // 当前帧的姿态角（来自原始imu数据）//第一帧第二个点云
            Eigen::Affine3f transBack = pcl::getTransformation(0, 0, 0, cloudInfo.imuRollInit, cloudInfo.imuPitchInit, cloudInfo.imuYawInit);
            // 当前帧相对于前一帧的姿态变换//点云的变换矩阵
    ...
    */
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) //防止不小心清空？
            {
    ...
    */
    ...
    /*  ICP算法
    迭代最近点（Iterative Closest Point）算法是一种点云匹配算法。
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
                    ...
                    //c=a×b=(a.x,a.y,a.z)×(b.x,b.y,b.z)=(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x)
    ...
    */
     ...
     * 公式推导：todo
    */
    // 详细细节：https://blog.csdn.net/weixin_37835423/article/details/111587379#commentBox
    ...
    */
    ...
    /**
     * 发布激光里程计
     * 在publishOdometry()里，注意pubLaserOdometryIncremental的话题是"lio_sam/mapping/odometry_incremental"，
     它发布的laserOdomIncremental位姿是经过IMU数据加权融合的，这个是由IMUPreintegration订阅的，用于制作高频的IMU里程计；
     而没有经过IMU数据融合，单纯由mapOptimization经过GTSAM优化后的位姿是由pubLaserOdometryGlobal发布的，它的话题是"lio_sam/mapping/odometry"，
     这个是由transformFusion订阅的，用于与IMU里程计进行融合。这两个位姿虽然数值可能区别不大，但是用途有着巨大的区别。
     注意laserOdomIncremental会有一个是否退化isDegenerate的判断，这个判断是在scan2map的LMOptimization那里计算匹配的J矩阵时获得的，
